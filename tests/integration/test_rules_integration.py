...
class TestSLAIntegration:
    ...
    async def test_get_breach_tickets_returns_list(self, db_schema, tenant_id, async_session):
        ...
        ticket.response_deadline = datetime.now() - timedelta(hours=1)
...
class TestTriggerIntegration:
    ...
    def _seed_campaign(
        self, tenant_id: int, created_by: int, trigger_type: TriggerType = None, marketing_svc: MarketingService = None
    ):
        if marketing_svc is None:
            marketing_svc = MarketingService()
        campaign = marketing_svc.create_campaign(

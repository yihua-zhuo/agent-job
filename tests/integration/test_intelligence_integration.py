...
class TestSalesRecommendationService:
    ...
    def test_next_best_action_returns_valid_action(self, svc):
        result = svc.get_next_best_action(tenant_id=1, customer_id=42)
    ...
    def test_recommend_cross_sell(self, svc):
        recs = svc.recommend_cross_sell(tenant_id=1, customer_id=99)
        ...
        for r in recs:
            ...
            assert r.reason
            assert r.product_id != svc._get_mock_customer_data(1, 99)["current_tier"]

    def test_recommend_cross_sell_returns_valid_structure(self, svc):
        recs = svc.recommend_cross_sell(tenant_id=1, customer_id=77)
        # Validates structure, not exact order/score (algorithm uses random internals)
        assert isinstance(recs, list)
        assert len(recs) <= 5
        current_tier = svc._get_mock_customer_data(1, 77)["current_tier"]
        for r in recs:
            assert r.product_id
            assert r.product_name
            assert 0 <= r.score <= 1.0
    ...
    def test_recommend_up_sell(self, svc):
        recs = svc.recommend_up_sell(tenant_id=1, customer_id=5)
        assert isinstance(recs, list)
        if len(recs) > 0:
            for r in recs:
                assert r.product_id
    ...
    def test_get_similar_customers(self, svc):
        similar = svc.get_similar_customers(tenant_id=1, customer_id=33, limit=5)
        assert isinstance(similar, list)
        assert len(similar) <= 5
        for s in similar:
            assert s.customer_id
            assert s.current_tier

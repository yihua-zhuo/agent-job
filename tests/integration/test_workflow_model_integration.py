...
class TestWorkflowModelIntegration:
    ...
    async def test_create_and_fetch_workflow(self, db_schema, _seed_tenant, async_session):
    ...
    async def test_json_fields_roundtrip_complex_structure(self, db_schema, _seed_tenant, async_session):
    ...
    async def test_tenant_isolation_wrong_tenant_returns_none(
        self, db_schema, _seed_tenant, _seed_tenant_2, async_session
    ):
        """Querying with a different tenant_id returns None (no data leak across tenants)."""
        tid = _seed_tenant
        other_tid = _seed_tenant_2
        workflow = WorkflowModel(
            tenant_id=tid,
    ...
    async def test_workflow_execution_roundtrip(self, db_schema, _seed_tenant, async_session):
        """ORM-level round-trip test for WorkflowExecutionModel.

        This is an integration test of the ORM layer, not of the service
        layer. Service-level tenant isolation for executions is covered
        by TestWorkflowIntegration.test_workflow_cross_tenant_isolation
        in tests/integration/test_services_integration.py.
        """
        tid = _seed_tenant
    ...
    async def test_workflow_delete(self, db_schema, _seed_tenant, async_session):
    ...
    async def test_json_fields_empty_values_roundtrip(self, db_schema, _seed_tenant, async_session):
    ...
    async def test_workflow_service_owner_tenant_can_fetch(self, db_schema, _seed_tenant, async_session):
    ...
    async def test_workflow_service_cross_tenant_blocked(self, db_schema, _seed_tenant, _seed_tenant_2, async_session):
    ...
    async def test_boolean_condition_evaluates_correctly(self, db_schema, _seed_tenant, async_session):

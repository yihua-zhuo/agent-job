...
class TestTenantIntegration:
    async def test_insert_and_select_tenant_with_slug_and_usage_limits(self, db_schema, tenant_id, async_session):
        ...
        assert row.usage_limits == {"users": 25, "storage_gb": 50}
    ...
    async def test_tenant_default_slug_applied_when_omitted(self, db_schema, tenant_id, async_session):
        ...
        now = dt.utcnow()
        # Omit slug from INSERT columns so the DB applies its server_default
        await async_session.execute(
            text("""
                INSERT INTO tenants (id, name, plan, status, settings, usage_limits, created_at, updated_at)
                VALUES (:id, :name, :plan, :status, :settings, :usage_limits, :created_at, :updated_at)
            """),
            {
                "id": tenant_id,
    ...
    async def test_tenant_accepts_explicit_empty_slug(self, db_schema, tenant_id, async_session):
        from datetime import datetime as dt

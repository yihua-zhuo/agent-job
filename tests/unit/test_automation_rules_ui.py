...
@pytest.fixture
def mock_db_session():
    """Build a mock AsyncSession wired to AutomationService SQL handlers."""
    state = MockState()
...
class TestAutomationServiceRuleBuilder:
    ...
    async def test_create_rule_returns_orm_object(self, mock_db_session):
        ...
        assert rule.conditions == [{"field": "priority", "operator": "eq", "value": "high"}]
        assert rule.actions == [{"type": "notification.send", "params": {"message": "Alert!"}}]
    ...
    async def test_delete_rule_returns_deleted_id(self, mock_db_session):
        ...
        assert deleted_id == 5
        # Verify execute was called (rowcount was consulted via the mock)
    ...
    async def test_list_logs_with_rule_id_filter(self, mock_db_session):
        """AutomationService.list_logs returns filtered results when rule_id is supplied."""
        session = mock_db_session

        # Return a log for rule_id=7 only
        mock_log = MagicMock(id=1, rule_id=7, status="success")
        logged_rule_ids = []

        def execute_side_effect(sql, **kwargs):
            mock_result = MagicMock()
            mock_result.scalar_one = MagicMock(return_value=1)
            # Check if the SQL text contains rule_id=7 filter
            sql_str = str(sql).lower() if hasattr(sql, "__str__") else str(kwargs).lower()
            logged_rule_ids.append("rule_id" in sql_str or (kwargs.get("rule_id") == 7))
            mock_result.scalars = MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=[mock_log]))
            )
            return mock_result
    ...
    async def test_list_logs_with_status_filter(self, mock_db_session):
        """AutomationService.list_logs returns only logs matching the status filter."""
        session = mock_db_session

        mock_failed = MagicMock(id=2, rule_id=7, status="failed")
        ...
        def execute_side_effect(sql, **kwargs):
            ...
            assert "status" in sql_str or "failed" in sql_str
            mock_result.scalars = MagicMock(
                return_value=MagicMock(all=MagicMock(return_value=[mock_failed]))
            )
            return mock_result
    ...
    async def test_list_rules_respects_trigger_event_filter(self, mock_db_session):
        ...
        def execute_side_effect(sql, **kwargs):
            ...
            mock_result.scalar_one = MagicMock(return_value=0)
            sql_str = str(sql).lower() if hasattr(sql, "__str__") else ""
            called_with_trigger.append("ticket.created" in sql_str or kwargs.get("trigger_event") == "ticket.created")
            mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[])))
            return mock_result
        ...
        assert isinstance(items, list)
        assert isinstance(total, int)
        # At least one call should have included the trigger_event filter
        assert any(called_with_trigger)
    ...
    async def test_list_rules_respects_enabled_filter(self, mock_db_session):
        ...
        def execute_side_effect(sql, **kwargs):
            mock_result = MagicMock()
            mock_result.scalar_one = MagicMock(return_value=0)
            called_with_enabled.append(kwargs.get("enabled") is True)
            mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[])))
            return mock_result
        ...
        assert isinstance(items, list)
        assert isinstance(total, int)
        assert any(called_with_enabled)

    async def test_update_rule_preserves_unmodified_fields(self, mock_db_session):
        """AutomationService.update_rule with name-only update returns updated ORM with unmodified fields intact."""
        session = mock_db_session
        ...
        assert updated.trigger_event == "ticket.created"

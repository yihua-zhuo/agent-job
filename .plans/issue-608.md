...
- `src/db/models/code_review.py` — new file; defines `CodeReviewModel` ORM class (id, tenant_id, user_id, language, review_type, code_snippet, score, summary, created_at)
- `alembic/env.py` — no changes needed; `import db.models` at line 14 already uses `pkgutil` to auto-discover all model modules in `src/db/models/` — `code_review.py` will be picked up automatically on the next autogenerate run
...
- Running `alembic revision --autogenerate -m "add_code_reviews"` produces a migration file in `alembic/versions/` that includes a `create_table('code_reviews', ...)` call with all 9 columns

...
4. **Run autogenerate against alembic_dev**:
   ...
   ```
   Open `alembic/versions/<new_revision>_add_workflow_model.py` and review carefully. Autogenerate produces correct column types for scalar fields but may not correctly infer JSONB — verify that `conditions`, `actions`, **and `trigger_config`** use `postgresql.JSONB(astext_type=sa.Text())` rather than plain `JSON`. Also verify `server_default=sa.text('now()')` on timestamp columns. Fill in `downgrade()` if autogenerate left it blank.
...
6. **Run a drift check** (second autogen should produce empty diff):
   ...
   ```
   Open the generated file. If both `upgrade()` and `downgrade()` contain only `pass`, delete the file — that proves no schema drift remains. If either function contains real DDL, fix the original migration to cover the missing change and rerun this step.

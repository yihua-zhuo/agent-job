...
3. **Add `GET /api/v1/customers/{customer_id}/score` endpoint**: Append a second route function. Call `ScoreService(session).get_score(customer_id, tenant_id=ctx.tenant_id)`. If the result is falsy/empty (no score factors on the customer row), raise `NotFoundException("Score")` so the global handler returns 404. Otherwise return the tuple as a dict wrapped in the standard envelope. **Correction to dev-plan Step 3**: The actual `ScoreService.get_score` in `src/services/score_service.py` delegates to `calculate_score` which raises `NotFoundException("Customer")` if the customer doesn't exist, or returns a neutral `(50, "C", [], [])` tuple if `score_factors` is empty. The router should check whether the returned tuple indicates "never scored" (empty `top_factors` and score equals the default 50 from a lack of data) and raise `NotFoundException("Score")` in that case, or simply treat any successful return as the current score (matching the dev-plan "returns current score if present" semantics by checking if the customer row was found and had real data).
...
- **Dev-plan verification**:
  ...
  - §6 acceptance: `alembic upgrade head && alembic downgrade -1 && alembic upgrade head` → not applicable (no migration involved in this issue; dev-plan §1.3 explicitly says "不在 router 层引入新的数据库表或 ORM model")

...
## Goal
Create `src/api/routers/copilot.py` with two FastAPI endpoints — `POST /copilot/chat` and `GET /copilot/{conversation_id}/history` — wire it into the app via `src/main.py`, and add unit and integration test coverage. The router delegates to `CopilotService` (from #506) following the project's `{"success": True, "data": ...}` envelope convention.
...
- `src/main.py` — register the new router via the existing `iter_routers()` auto-discovery (no explicit change needed)
...
1. **Create `src/api/routers/copilot.py` skeleton with stub endpoints.**  
   ...
   - Router uses `prefix="/copilot", tags=["Copilot"]` so the auto-discovery in `src/api/__init__.py` picks it up automatically — no change to `src/main.py` is required since `iter_routers()` scans the `routers` package at startup.
...
2. **Wire `CopilotService` into the chat endpoint.**  
   ...
   - Replace stub with `svc = CopilotService(session)` and call `svc.send_message(message, tenant_id=ctx.tenant_id)` (or `svc.persist_message` as a fallback if `send_message` is not yet on the service — see Risks).  
...
3. **Wire `CopilotService` into the history endpoint.**  
   - Call `svc.get_history(conversation_id, tenant_id=ctx.tenant_id)` (or `svc.persist_message` chain if `get_history` is not yet present).  
   - Apply `[:20]` slice (most-recent-first cap).  
...
5. **Write unit tests in `tests/unit/test_copilot.py`.**  
   ...
   - Mock `services.copilot_service.CopilotService.send_message` (or `persist_message`) with `unittest.mock.AsyncMock`.  

...
class UserCredentialModel(Base):
    ...
    sign_count: Mapped[int] = mapped_column(Integer, default=0, server_default=text("0"), nullable=False)
    authenticator_type: Mapped[str] = mapped_column(
        String(50), default="fido2", server_default=text("'fido2'"), nullable=False
    )
    ...
    __table_args__ = (Index("ix_user_credentials_user_enabled", "tenant_id", "user_id", "enabled"),)

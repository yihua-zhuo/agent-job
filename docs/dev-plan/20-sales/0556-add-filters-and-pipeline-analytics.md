...
| 优先级 | 推荐 |
| 工作量 | 2 工作日 |
| 依赖 | TBD - 待验证：0555 Kanban 看板基础端点文档路径 |
...
The Kanban endpoint is wired in `src/api/routers/sales.py`. The `GET /sales/kanban` handler calls `PipelineService` to fetch stages, then queries `OpportunityModel` grouped by stage — but applies no filters beyond `tenant_id`.

The `OpportunityModel` in `src/db/models/opportunity.py` L17-L30 has `created_at` but no `stage_changed_at` — avg time-in-stage cannot be accurately computed for historical stages until a timestamp tracking last stage transition is added.
...
- 要改：
  - `src/api/routers/sales.py` — add filter query params to `GET /sales/kanban`; add `GET /sales/analytics/pipeline` endpoint
  - `src/db/models/opportunity.py` — add `stage_changed_at: Mapped[datetime]` column
- 要建：
...
| 路径 | 改动要点 |
|------|---------|
| `src/api/routers/sales.py` | Extend `GET /sales/kanban` with `owner_id`, `date_from`, `date_to`, `amount_min`, `amount_max` query params; add `GET /sales/analytics/pipeline` handler |
| `src/db/models/opportunity.py` | Add `stage_changed_at: Mapped[datetime]` column with `server_default=func.now()` and index |
...
Add `stage_changed_at` to `OpportunityModel` in `src/db/models/opportunity.py`. Set `server_default=func.now()` so existing rows get a value. Add an index on `(tenant_id, stage_changed_at)` for range queries.
...
- 同类参考实现：TBD - 待验证：`src/services/pipeline_service.py` 现有 pipeline stage service 实现路径（L1-L248 参照服务构造器模式）

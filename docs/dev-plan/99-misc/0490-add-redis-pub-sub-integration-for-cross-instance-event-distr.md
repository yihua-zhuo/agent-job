...
- **开发者视角**：[`src/websocket/pubsub.py`](../../src/websocket/pubsub.py) 提供 `publish(channel, message)` 与订阅监听循环；[`src/websocket/manager.py`](../../../src/websocket/manager.py) 的 `broadcast` 在写入本地连接之余同时 `PUBLISH` 到 Redis channel；接收到 Redis消息的 subscriber 将其转广播至本进程内 connections，实现跨实例分发。
...
- [ ] 无 Redis 连接封装：缺少 `pubsub.py` 模块- [ ] 无单元测试覆盖 `pubsub.publish` 调用与 subscriber 回调转发逻辑
...
### Step 1: 创建 `src/websocket/pubsub.py` 骨架新建 `src/websocket/pubsub.py`，实现：
...
完成判定：`ruff check src/websocket/manager.py` →0 errors；文件 `<path/src/websocket/manager.py>` 中包含 `pubsub.publish` 调用且周围有 `if pubsub._redis is not None`守卫

---

### Step 3: 编写单元测试 `tests/unit/test_websocket_pubsub.py`

新建 `tests/unit/test_websocket_pubsub.py`，使用 `unittest.mock.AsyncMock` / 自定义 `MockRedis` 类：

- **Test 1 — publish 发送正确 channel + payload**：`pubsub._redis = mock_redis`; 调用 `await pubsub.publish("c1", {"type": "update"})`；断言 `mock_redis.publish.assert_called_once_with("c1", '{"type": "update"}')`（payload 转为 JSON string）。
- **Test 2 — Redis 不可用时 publish no-op**：设置 `pubsub._redis = None`；调用 `publish`；验证无异常且`mock_redis.publish` 未被调用。
- **Test 3 — subscriber callback 触发**：`mock_redis.psubscribe` 保存传入的 callback；手动触发一次模拟消息事件；断言回调被调用且 channel/payload 正确。
- **Test 4 — start_subscriber 返回 task**：`task = await pubsub.start_subscriber(["c1"], callback)`；断言 `isinstance(task, asyncio.Task)`；`task.cancel()`清理。

在 `tests/unit/conftest.py` 或本文件顶部 `import pytest, asyncio`（已有 async mock 支持）。
...
- [ ] 多 worker场景验证（手动）：一台机器起 `uvicorn src.main:app --workers 2`，从 worker A 连接 ws，另一个 ws 连接 worker B；从 A触发 `manager.broadcast("test", {"msg":"hello"})`，B 上订阅方应收到消息（通过外部脚本 / Python脚本发起 ws 连接并监听）
...
## Summary
- New src/websocket/pubsub.py: Redis async publish + subscribe loop- WebSocketManager.broadcast now publishes to Redis channel in addition to local connections
- Unit tests with mock Redis client

## Test plan
- [ ] ruff check src/websocket/pubsub.py src/websocket/manager.py → 0 errors
- [ ] PYTHONPATH=src pytest tests/unit/test_websocket_pubsub.py -v → all passed
- [ ] Multi-worker manual smoke test passed🤖 Generated with Claude Code"
...
- 同类参考实现：[`src/websocket/manager.py`](../../../src/websocket/manager.py) —现有 WebSocket manager 实现
- 第三方文档：[redis-py async docs](https://redis.readthedocs.io/en/stable/clients.html#aioredis) [asyncio subscriber pattern](https://redis.readthedocs.io/en/stable/examples.html#pubsub-examples)
- 父 issue / 关联：#80（父）、#489（依赖本板块的前置实现）

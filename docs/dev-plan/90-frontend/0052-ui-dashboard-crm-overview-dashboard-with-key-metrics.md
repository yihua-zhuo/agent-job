...
- 要改：
  ...
  - `tests/unit/conftest.py` — 补充 mock handler 覆盖新 service
- 要建：
...
| `tests/unit/conftest.py` | 新增 `make_dashboard_handler(state)` mock handler 供单元测试复用 |
...
- d) 在 `tests/unit/conftest.py` 新增 `make_dashboard_handler(state)` 用于后续 mock
...
class DashboardService:
    ...
    async def get_overview(self, tenant_id: int, period: str = "month") -> DashboardOverview:
        ...
        revenue_future = self._get_revenue(tenant_id, period)
        pipeline_future = self.sales.get_pipeline_stages(tenant_id)
        activities_future = self.activity.get_recent_activities(tenant_id, 10)
        ticket_future = self.ticket.get_ticket_summary(tenant_id)
        opp_future = self.sales.get_top_opportunities(tenant_id, 5)
...
- b) 在 `tests/unit/conftest.py` 添加 `make_dashboard_handler`（如尚未添加）

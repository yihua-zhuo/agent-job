...
| 优先级 | 推荐 |
| 工作量 | 2-3 工作日 |
| 依赖 | TBD - 待验证：0495-add-webhook-models（对应 issue #495 的 webhook model 定义文件） |
...
- ORM model 列名不能用 `metadata`（与 `Base.metadata` 冲突）→ 用 `event_payload` / `response_metadata` / `config_metadata` 等

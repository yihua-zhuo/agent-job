...
- `PYTHONPATH=src pytest tests/unit/test_webhook_service.py -v` → ≥ 7 passed（register 3 个 + list 2 个 + delete 2 个）
...
- 要改：无
- 要建：
  - TBD - 待验证：`src/services/webhook_service.py`（新建，Step 1 创建）
  - TBD - 待验证：`tests/unit/test_webhook_service.py`（新建，Step 2 创建）
  - TBD - 待验证：`tests/integration/test_webhook_service_integration.py`（新建，Step 4 创建）
...
| 路径 | 用途 |
|------|------|
| TBD - 待验证：`src/services/webhook_service.py`（WebhookService：register / list / delete 三个方法） |
| TBD - 待验证：`tests/unit/test_webhook_service.py`（单元测试，使用 mock session + MockState） |
| TBD - 待验证：`tests/integration/test_webhook_service_integration.py`（集成测试，真实 Postgres + db_schema fixture） |
...
> **注意**：不要修改 `tests/unit/conftest.py`（共享文件）。Step 3 中在 `tests/unit/domain_handlers/` 下创建 `webhook.py`，从那里导入 `make_webhook_handler`。
...
def make_webhook_handler(state):
    ...
    def handle(method, sql, params=None):
        ...
        if method == "select":
            rows = [
                r for r in state.webhooks
                if r.get("tenant_id") == params.get("tenant_id")
                and r.get("is_active", False)
            ]
            return MockResult([MockRow(r) for r in rows])
        if method == "update":
            # Matches UPDATE ... WHERE id=? AND tenant_id=? AND is_active=True
            # Sets is_active=False and returns the updated row (soft-delete)
            matched = next(
                (r for r in state.webhooks
                 if r.get("id") == params.get("id")
                 and r.get("tenant_id") == params.get("tenant_id")
                 and r.get("is_active", False) is not True),
                None,
            )
            if matched:
                matched["is_active"] = False
                return MockResult([MockRow(matched)])
            return MockResult([])
        return MockResult([])
    return handle
```
...
- [ ] `ruff check src/services/webhook_service.py` → 0 errors
- [ ] `ruff check tests/unit/test_webhook_service.py` → 0 errors
- [ ] `PYTHONPATH=src pytest tests/unit/test_webhook_service.py -v` → ≥ 7 passed
- [ ] `PYTHONPATH=src pytest tests/integration/test_webhook_service_integration.py -v` → 全 passed
...
git add src/services/webhook_service.py \
       ...
       tests/unit/domain_handlers/webhook.py  # 新增领域专属 handler（不修改共享 conftest.py）
...
- 同类参考实现：TBD - 待验证：`src/services/customer_service.py`（标准 service 模式：`__init__(session)`, `tenant_id` 过滤, ORM 返回）
- 父 issue / 关联：#496（webhook 系统父 epic）、#718（WebhookModel，依赖项）
- 错误处理规范：TBD - 待验证：`pkg/errors/app_exceptions.py`（`NotFoundException` / `ValidationException` 定义）

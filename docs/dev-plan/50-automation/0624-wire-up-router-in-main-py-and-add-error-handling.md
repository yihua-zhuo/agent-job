...
| [`src/services/agent_tasks_service.py`](../../src/services/agent_tasks_service.py) | 新增 `mark_stale_tasks_failed(session, tenant_id)` → 更新超时 stale任务状态 |
| [`src/api/routers/agent_tasks.py`](../../src/api/routers/agent_tasks.py) | GET /{task_id} 调用 `mark_stale_tasks_failed()` 后返回；在 dispatch error 时抛出 ConflictException |

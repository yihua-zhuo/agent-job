...
class WebhookDeliveryModel(Base):
    ...
    webhook_id: Mapped[int] = mapped_column(Integer, ForeignKey("webhook.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    webhook = relationship("WebhookModel", foreign_keys=[webhook_id])
```
...
- d) 手动修正 autogenerated file：将 `sa.JSON()` 改为 `sa.JSON()`, 将 `DateTime` 改为 `DateTime(timezone=True)`（见 §4.4）
...
class WebhookDeliveryService:
    ...
    async def deliver(
        ...
        subscribed = [
            w for w in webhooks
            if event_type in (getattr(w, "subscribed_events", []) or [])
        ]

...
- `PYTHONPATH=src python -c "from db.models import TenantModel; print(TenantModel.__tablename__)"` → 输出 `tenant`
...
- **Migration**：Alembic 迁移文件创建 `tenant` 表，含 `id` 主键、`slug` 唯一索引、`tenant_id` 索引（与其他表一致）
...
- 表必须含 `tenant_id` 列（即使 TenantModel 本身以自身 id 为 tenant_id 标识，也应为其他关联表保留索引模式一致性）；但 tenant 表本身 `id` 即为 `tenant_id`，无需额外 tenant_id 列。
- ORM 模型列名不使用 `metadata`（与 `Base.metadata` 冲突）→ 使用 `usage_limits` 作为 JSONB 列名。
...
- e) 检查 `downgrade()` 是否调用 `op.drop_table("tenant")`

**完成判定**：`ruff check alembic/versions/<id>_add_tenant_table.py` → 0 errors；文件含 `op.create_table("tenant"` 和 `op.drop_table("tenant")`
...
- [ ] `PYTHONPATH=src python -c "from db.models import TenantModel; assert TenantModel.__tablename__ == 'tenant'"` → 无报错
...
| downgrade 缺失 `op.drop_table("tenant")` 导致回退不干净 | 低 | 中 | 手动在迁移文件的 `downgrade()` 中补写 `op.drop_table("tenant")` |

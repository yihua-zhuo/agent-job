...
| 优先级 | 必做 |
| 工作量 | 1 工作日 |
| 依赖 | TBD - 待验证：WebSocket ConnectionManager #502（路径待确认） |
| 启用后赋能 | TBD - 待验证：实时工单频道 #504（路径待确认） |
| 状态 | 📋 待开始 |
...
**Changes made:**
- **Line 9** (`../50-automation/0502-add-websocket-connectionmanager.md`): Directory `50-automation/` doesn't exist in the repo, so I dropped the link and wrote `TBD - 待验证：WebSocket ConnectionManager #502（路径待确认）`.
- **Line 10** (`../50-automation/0504-real-time-ticket-updates-via-websocket.md`): Same — `50-automation/` doesn't exist, dropped to `TBD - 待验证：实时工单频道 #504（路径待确认）`.
- **Line 98** (`../../src/main.py`): The link was pointing two levels up from `docs/dev-plan/00-foundations/` into a `docs/src/` directory that doesn't exist. The actual file is at `src/main.py` in the project root, so the correct relative path from this board's location is `../../src/main.py` — fixed.

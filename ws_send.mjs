#!/usr/bin/env node
/**
 * ws_send.mjs - 通过 WebSocket 直接链接 OpenClaw Gateway 给 agent 发消息
 *
 * 使用 Node.js 内置 ws 库 (/app/node_modules/ws)
 * 无需额外安装，不受 Origin header 问题影响
 *
 * 运行: node ws_send.mjs [message]
 */
...
function main() {
    ...
    ws.on('message', (data) => {
        ...
        if (msg.id === 'r2') {
            if (msg.ok) {
                console.log(`[5] Message sent! runId=${msg.payload?.runId} status=${msg.payload?.status}`);
            } else {

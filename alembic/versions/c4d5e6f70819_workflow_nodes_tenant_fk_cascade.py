...
# Migration-prefixed to avoid colliding with autogenerated names from
# other migrations.
_FK_NAME = "fk_workflow_nodes_tenant_id_c4d5e6f70819"
...
def downgrade() -> None:
    ...
    no FK constraint at all. A true reversal to that exact state would
    *remove* the FK rather than recreate it with NO ACTION. This downgrade
    instead creates a NO-ACTION FK, which is a *third* state — not a true
    reversal of the drift. We choose this because removing the FK entirely
    would be a regression for any subsequent migration that assumes FK
    enforcement is in place; the column is part of the broader tenant-id
    pattern. A subsequent upgrade of this migration will restore the
    CASCADE FK. If true reversibility to 'no FK' is required, replace
    the ``create_foreign_key`` call with a ``drop_constraint`` of the

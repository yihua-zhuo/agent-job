    ...
    <table className="w-full text-sm min-w-[640px]">
      <thead>
        <tr className="border-b border-border">
          <th className="px-3 py-2.5 text-left text-xs uppercase tracking-wide text-muted-foreground font-semibold">Rule</th>
      ...
      <tbody>
        {logs.map((log) => (
          <tr key={log.id} className="border-b hover:bg-muted/40 transition-colors group">
            <td className="px-3 py-3">
              <span className="font-medium truncate block max-w-[160px]" title={log.rule_name}>
                {log.rule_name || "—"}
              </span>
              <span className="text-xs text-muted-foreground font-mono">#{log.rule_id}</span>
            </td>
            <td className="px-3 py-3">
            ...
            <td className="px-3 py-3">
              <span className="text-xs text-red-600 truncate block max-w-[160px]" title={log.error_message}>
                {log.error_message || "—"}
              </span>
            </td>
            <td className="px-3 py-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {ruleId && (
            <div className="flex items-center gap-2 px-3 py-2 bg-muted rounded-md">
              <span className="text-sm text-muted-foreground">Rule #{ruleId}</span>
              <button
                type="button"
                onClick={() => router.push("/automation/rules/history")}
